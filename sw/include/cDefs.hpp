...
/* IOCTL 
    * Define cmd numbers for the ioctl interface of the driver/kernel module (`fpga_ioctl()`).
    * Each cmd number is a unique magic number generated by combining the 1st and 2nd arguments.
    * `_IOW` allows for writes from userspace (the app) to kernel space (the kernel module) interaction.
    * `_IOR` enables reading data from (the kernel module) kernel space.
    * The 3rd argument stands for the communicated data type.
*/
